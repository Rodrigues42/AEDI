...
struct tNo * removerAntecessor(struct tNo *r);
...
void preOrder(struct tNo *r, struct tNo *ultimo)
{
    if(r != NULL)
    {
        printf("%c%s", r->chave, (ultimo == r && r->direita == NULL && r->esquerda == NULL) ? "\n" : " ");
...
struct tNo * removerAntecessor(struct tNo *r)
{
    ...
    if(r->esquerda == NULL && r->direita == NULL) // é folha
    {
        if(pai->esquerda == r)
        {
            pai->esquerda = NULL;
    ...
    else // tem um filho (nesse caso, à esquerda, pois é o maior da subárvore esquerda)
    {
        if(pai->esquerda == r)
        {
            pai->esquerda = r->esquerda;
        }
        else
        {
            pai->direita = r->esquerda;
        }
