...
struct tNo * antecessor(struct tNo *r);
struct tNo * removerSucessor(struct tNo *);
struct tNo * imediatamenteMaior(struct tNo *);
struct tNo * imediatamenteMenor(struct tNo *);
...
int main()
{
    ...
    {
        scanf("%d", &chave);
        struct tNo * chaveExistente = buscar(arvore, chave);

        if (chaveExistente != NULL && chaveExistente->chave == chave)
        {
            // Já Existe e é comando para exibir
            if (strcmp(opc, "immediately-greater") == 0)
            {
                struct tNo * maior = imediatamenteMaior(chaveExistente);
                if (maior != NULL && maior->chave != chave)
                {
                    printf("%d\n", maior->chave);
                }
                else
            ...
            {
                struct tNo * menor = imediatamenteMenor(chaveExistente);
                if (menor != NULL && menor->chave != chave)
                {
                    printf("%d\n", menor->chave);
                }
                else
...
struct tNo * removerSucessor(struct tNo *r)
{
    ...
    else // eh pai de um
    {
        ...
        if (r->esquerda != NULL)
            r->direita->pai = pai; // tem que ver se nao vai dar RTE (Corrigido verificar)
    }

    return r;
}

struct tNo * imediatamenteMaior(struct tNo *r)
{
    struct tNo * pai = NULL;
    r = r->direita;
    while(r != NULL)
...
struct tNo * imediatamenteMenor(struct tNo *r)
{
    struct tNo * pai = NULL;
    r = r->esquerda;
    while(r != NULL)
