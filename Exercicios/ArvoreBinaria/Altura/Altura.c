...
int altura(struct tNo *r);

int main()
{
    int chave, n;
    struct tNo *chaveExistente;
    struct tNo *it;
    ...
    {
        scanf("%d", &chave);
        chaveExistente = buscar(arvore, chave);
        if (buscar(arvore, chave) != NULL && chaveExistente->chave == chave)
        {
        }
        else
        {
...
{
    if (r == NULL)
    return -1; // ou 0, dependendo da definição (nós ou arestas)
